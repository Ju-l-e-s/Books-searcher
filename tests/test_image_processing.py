...
class TestImageProcessor:
    ...
    def test_preprocess_crop_rotates_horizontal_spines(self, processor):
        """Wide (horizontal) crops should be rotated to vertical."""
        horizontal_crop = np.zeros((50, 200, 3), dtype=np.uint8)
        result = processor._preprocess_crop(horizontal_crop)
        # ROTATE_90_CLOCKWISE: (h, w) -> (w, h) = (200, 50)
        assert result.shape == (200, 50)

    def test_preprocess_crop_vertical_spines_untouched(self, processor):
        """Tall (vertical) crops should NOT be rotated."""
        vertical_crop = np.zeros((200, 50, 3), dtype=np.uint8)
        result = processor._preprocess_crop(vertical_crop)
        # Already portrait (h > w), so no rotation.
        # After grayscale: shape = (200, 50) → h=200, w=50 → h > w (portrait)
    ...
    def test_ocr_crop_extracts_text(self, processor):
        ...
        processor._reader.readtext.assert_called_once_with(crop, detail=0)
        ...
        with patch("app.image_processing.YOLO") as mock_yolo:
            ...
            def fake_exists(self):
                if "book-spines.pt" in str(self):
                    return False  # Custom model not found → fallback
